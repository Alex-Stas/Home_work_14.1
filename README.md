...
Module category.py with class Category объявляет класс Category c категорией товаров и полями:
    name - название категории
    description - описание категории
    products_list - список товаров в данной категории
    дополнительно два глобальных значения 

    так


filter_transaction_by_word - Функция фильтрует список транзакций по наличию заданной подстроки
    (может быть задано и регулярное выражение) в определенном поле, по умолчанию в description
    и выдает новый список только с заданными значениями. Встроен дополнительный фильтр на операции
    с отсутствующими ключами - исключение транзакций с KeyError и одновременно проверка на формат
    файла - выводится сообщение и пустой список.

Module transaction_statistics.py with function:

transaction_statistics_by_field - Функция принимает список словарей с данными о транзакциях
    и поле для получения статистики по нему. Возвращает словарь, в котором ключи — это названия
    категорий, а значения — количество операций в каждой категории по выбранному полю.
    По умолчанию (основной функционал) выводятся категории операций из поля description
    Встроен дополнительный фильтр на операции с отсутствующими ключами - исключение транзакций
    с KeyError и одновременно проверка на формат файла - выводится сообщение и пустой список.
    
Module main.py with functions:
select_file - Функция выдает приветствие и предлагает выбрать из какого файла загрузить
    возвращаемый список транзакций, далее вызывает соответствующую функцию
status_selection - Функция выдает сообщение и предлагает выбрать статус для фильтрации
    возвращаемого списка транзакций. Вызывает функцию filter_transaction_by_word из модуля
    transaction_filters.py
date_sorting - Функция выдает сообщение сортировать ли список по датам, если да, то уточняет
    по возрастанию или убыванию и возвращает отсортированный список, если нет,
    то список возвращается без изменений
rouble_filter - Функция выдает сообщение выводить ли только рублевые транзакции из списка,
    если да, то вызывает программу фильтр и возвращает отфильтрованный список, если нет,
       то список возвращается без изменений. Вызывает функцию filter_transaction_by_word из модуля
    transaction_filters.py
word_filter - Функция выдает сообщение фильтровать ли транзакции из списка, по определенному слову
    ...
    Вызывает функцию filter_transaction_by_word из модуля transaction_filters.py
output_print - Функция выдает выбранные на предыдущих этапах транзакции по определенному формату
    с преобразованием даты и сообщением, если список пустой
flatten_dict - Функция для обработки списка транзакций из JSON файла - удаление иерархии в сумме транзакций
    скопирована как готовая с сайта sky pro
flatten_list - Функция для обработки списка транзакций из JSON файла - переименование полей в стандартные
    после удаления иерархии в сумме транзакций. Вызывает функцию flatten_dict.
main - Функция последовательно запускает функции запросов и обработки списка транзакций
    с ветвлением в случае необходимости удаление иерархии словарей в сумме в списке из json и
    приведение полей к стандартному виду для совместимости дальнейшей обработки

***END OF NEW INFO DTD 07/05/2025***

Module utils_csv.py with function:

get_transactions_from_csv_file - Функция принимает на вход путь до CSV-файла с транзакциями
    (c ';' в качестве разделителя) и возвращает список словарей с данными о финансовых транзакциях.
    Если файл не найден или пустой, функция возвращает пустой список,
    в случае отсутствия файла дополнительно выводится сообщение об ошибке.

Module utils_excel.py with function:

get_transactions_from_excel_file - Функция принимает на вход путь до excel-файла с транзакциями
...
utils_csv.py:
    ...
    не выполняет большого объема обработки и настройка тестирования корректировкой файлов
    гибче, чем подменой._
        Тестирование обработки корректных данных
        Тестирование обработки отсутствия файла, вывод сообщения и пустого списка
        Тестирование обработки пустого файла - вывод пустого списка
...
transaction_filters.py
    filter_transaction_by_word:
        Тестирование основных случаев фильтрации
        Тестирование случая отсутствия аргумента field, default - значение
        Тестирование случая отсутствия ключей для поиска
        Тестирование случая некорректного формата списка транзакций